...
const Home = () => {
  ...
  const fetchHomeData = async () => {
    try {
      setLoading(true);
      const [tournamentsRes, teamsRes, matchesRes, annRes] = await Promise.all([
      ...
      const announcementsList = annRes.data?.announcements || [];
    ...
    <div className="relative min-h-screen text-slate-100 overflow-x-hidden">
      ...
      <div className="relative z-10 space-y-6 sm:space-y-8 pb-12 pt-2 sm:pt-4">
        ...
        <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 space-y-8 sm:space-y-10">
          ...
          <section className="space-y-4">
            ...
            {announcements.length === 0 ? (
              <div className="p-8 rounded-2xl bg-slate-900/40 border border-slate-800/80 text-center space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Official club communications will appear here.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                {announcements.map((ann) => {
                  const hasImage = ann.image && ann.image.trim().length > 0;
                      ...
                      {hasImage && (
                        <div className="h-40 w-full overflow-hidden relative bg-slate-950">
                          <img
                            ...
                            onError={(e) => {
                              const container = e.target.closest('.overflow-hidden');
          ...
          {/* 9. QUICK PORTAL HUBS */}
          {/* ========================================================= */}
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 pt-2">
            <Link
              to="/tournaments"
            ...
            >
              <div className="space-y-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 line-clamp-2">
                  Browse open registrations and register your squad.
                </p>
              </div>
            ...
            <Link
              to="/news"
              className="p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-indigo-950/30 to-slate-900 border border-indigo-500/30 hover:border-indigo-500/60 transition-all group flex items-center justify-between gap-3 shadow-lg"
            >
              <div className="space-y-1 min-w-0">
                <span className="text-[10px] uppercase font-mono font-bold text-indigo-400">Official Updates</span>
                <h3 className="text-base sm:text-lg font-black text-white font-mono group-hover:text-indigo-300 transition-colors truncate">
                  ANNOUNCEMENTS
                </h3>
                <p className="text-xs text-slate-400 line-clamp-2">
                  Club bulletins, rules, and schedule updates.
                </p>
              </div>
              <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400 group-hover:translate-x-1 transition-transform shrink-0">
                <ArrowRight className="w-5 h-5" />
              </div>
            </Link>
            ...
            >
              <div className="space-y-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 line-clamp-2">
                  View team points, victories, and rankings.
                </p>
              </div>
